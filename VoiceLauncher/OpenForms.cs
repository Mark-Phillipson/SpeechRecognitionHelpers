    ...
    public class OpenForms
    {
        ...
        public void LoadForm()
        {
            ...
            if (args.Count() < 2)
            {
                ...
                // arguments = new string[] { args[0], "Not Applicable", "Folders" };
            }
            else
            {
            ...
            else if (arguments[1].EndsWith("Launcher") )
            {
                LauncherForm launcherForm = new LauncherForm();
                Application.Run(launcherForm);
                return;
            ...
            else
            {
                ...
                Category category = db.Categories.Where(v => v.CategoryName == categoryName && v.CategoryType == "IntelliSense Command").FirstOrDefault();
                if (category == null)
                {
                ...
                //MessageBox.Show($"1:{arguments[1]} 2:{arguments[2]} Language:{language?.LanguageName} Category:{category?.CategoryName}");
                Application.Run(customIntelliSense);
            }
        }
    }
