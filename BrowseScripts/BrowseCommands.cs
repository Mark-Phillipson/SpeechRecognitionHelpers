...
namespace BrowseScripts
{
    public partial class BrowseCommands : Form
    {
        ...
        private void SetUpFormAccordingToCommandlineArguments()
        {
            ...
            if ((args.Count() > 1 && args[1] == "ManagedLists"))
            {
                var form = new ManageLists();
                if (args.Count() > 2)
                {
                    form.Filter = args[2];
                }
        ...
        private void SetUpCommand()
        {
            ...
            dataGridViewCommand.Columns[2].Width = 40;
            dataGridViewCommand.Columns[3].HeaderText = "Enabled?";
            dataGridViewCommand.Columns[3].Width = 60;

            DataGridViewComboBoxColumn applicationComboboxColumn = new DataGridViewComboBoxColumn();
            applicationComboboxColumn.Name = "applicationComboboxColumn";
            applicationComboboxColumn.HeaderText = "Application";
            applicationComboboxColumn.DataSource = dataSet.Tables[1];
            applicationComboboxColumn.Name = "Commands_Id";
            applicationComboboxColumn.DataPropertyName = "Commands_Id";
            applicationComboboxColumn.DisplayMember = "moduleDescription";
            applicationComboboxColumn.ValueMember = "Commands_Id";
            var style = new DataGridViewCellStyle();
            style.BackColor = Color.Black;
            style.ForeColor = Color.White;
            applicationComboboxColumn.DefaultCellStyle = style;
            applicationComboboxColumn.FlatStyle = FlatStyle.Flat;
            dataGridViewCommand.Columns.Add(applicationComboboxColumn);
            dataGridViewCommand.Columns[4].DefaultCellStyle = style;
            dataGridViewCommand.Columns[4].Width = 200;
            for (int i = 0; i < 5; i++)
            {
        ...
        private void toolStripMenuItemManageLists_Click(object sender, EventArgs e)
        {
            var form = new ManageLists();
            form.Filter = GetListName();
            form.Show();
        }
        private string GetListName()
        {
            ...
            string commandName = currentRowDRV.Row.ItemArray[Mapping.ScriptName].ToString();
            if (currentRow != null)
            {
                if (ListManagement.HasLists(commandName))
                {
                    List<string> lists = ListManagement.GetListsName(commandName);
                    return lists[0];
                }
            }
            return "";
